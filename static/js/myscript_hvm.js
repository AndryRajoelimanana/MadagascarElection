...
var partition = d3.layout.partition()
    ...
    // .attr("tot", function(d) { return d.tot; });
//    .total(function(d) { return d.size; })
...
d3.json("static/js/result_avana.json", function(err, data) {
    avanajson = data;
    var partition_avana = d3.layout.partition()
        .size([2 * Math.PI, radius * radius])
        .value(function(d) { return d.size; });

    nodes_avana = partition_avana.nodes(avanajson);
            //.filter(function(d) {
            //    return (d.dx > 0.0000000005); // 0.005 radians = 0.29 degrees
            //});
});



d3.json("static/js/result_hvm.json", function(err, data) {
    hvmjson = data;
...
function createVisualization(json, svgin) {
    ...
    var path = svgin.data([json]).selectAll("path")
        ...
        .attr("d", arc)
        // .attr("indexc", function(d) { return d.index();})
...
function mouseover(d) {
    //node_avana = searchTree(nodes_avana, d.name, d.depth);
    // node_avana = nodes_avana.filter(node => (node.name == d.name) & (node.depth == d.depth))[0];
    index_node = nodes.indexOf(d);
    console.log(d.name);    
    var percentage = (100 * d.value / (nodes_avana[index_node].value + d.value)).toPrecision(4);
...
function searchTree(element, matchingTitle, depth){
    var x;
    for (x in element) { 
        if(element[x].name == matchingTitle && element[x].depth == depth){
            return element[x];
        }
    }
    return null;
...
//function searchTree(element, matchingTitle, depth){
//     if(element.name == matchingTitle && element.depth == depth){
//          return element;
//     }else if (element.children != null){
//          var i;
//          var result = null;
//          for(i=0; result == null && i < element.children.length; i++){
//               result = searchTree(element.children[i], matchingTitle, depth);
//          }
//          return result;
//     }
//     return null;
